...
/*   Updated: 2018/07/09 13:21:19 by msharpe          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_listdirectory(char *path, t_ls_stru *ls_list)
{
	...
	while((dir = readdir(d)) != NULL) // read directory going forward
	{
		...
		//TODO update while statement to go until head->next == null) -->to run until orderit's linked list is done
		if (yes == 1 && dir->d_name[0] != '.') //yes->->value once order_it is functional ||| Change dir->d_name to linkedlist ->path
		{
			if(ls_list ->l_flag == 1)
				ft_fullinfo(dir->d_name, ls_list);	
			if(ls_list->cap_r_flag == 1)
			{
				head = ft_savedaname(bits, ls_list, head);//add "filename" to linked list
				ft_printf("{cyan}%s\n", dir->d_name);
			}
		}
		...
		//TODO cycle to the next linked list node
	}
